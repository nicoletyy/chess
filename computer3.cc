...
ComputerLevel3::ComputerLevel3(Colour colour): Player{colour, 0} {}

void ComputerLevel3::move(int start_r, int start_c, int end_r, int end_c) {
...
// If it is a capturing move, score plus one;
// If the moving piece would be captured, score minus one;
// If the moving piece would avoid capturing, score plus one;
// If the move checks, it takes priority. 
void ComputerLevel3::move() {
	...
	for(int i = 0; i < (int)pieces.size(); i++) {
		...
		int col = pieces[i]->getInfo().col;
                // check if moving the piece avoids capturing
                int avoidCapScore = 0;
                if(pieces[i]->getLocation()->underAttack()) avoidCapScore = 1;
